...
from ada_utils.augment import AugmentPipe
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in range(args.iter):
        ...
        real_pred = discriminator(augment_pipe(real_img))  # Apply ADA augmentation
...
def evaluation(generator, args, steps):
    cnt = 0
    print(f" entering evaluation function ")
    for _ in tqdm(range(args.val_num_batches)):
        print(f" range of validation batches : {range(args.val_num_batches)} ")
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print(f" langth of the dataset : {len(loader.dataset)}")
    if len(loader.dataset) < args.batch:
        print(args.batch)
        print(len(loader.dataset))
        args.batch = len(loader.dataset)
    if len(loader.dataset) < args.val_batch_size:
        print(args.val_batch_size)
        args.val_batch_size = len(loader.dataset)


    if get_rank() == 0 and wandb is not None and args.wandb:
        wandb.init(project=args.project_name)

    print(f"Training with batch size: {args.batch}")
    print(f"Validation with batch size: {args.val_batch_size}")
    print(f"Number of validation batches: {args.val_num_batches}")
