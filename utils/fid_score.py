...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu', num_workers=8):
    ...
    print("dataset will be loading now")
    dataset = ImagePathDataset(files, transforms=TF.ToTensor())
    print("code working after the warning")
    dataloader = torch.utils.data.DataLoader(dataset,
                                             batch_size=batch_size,
                                             shuffle=False,
                                             drop_last=False,
                                             num_workers=num_workers)
    print(f"Dataload done")
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=8):
    ...
    for p in paths:
        ...
        if not os.path.exists(p):
            ...
            files = [os.path.join(p, f) for f in os.listdir(p) if f.lower().endswith(tuple(IMAGE_EXTENSIONS))]
